    ...
    internal class DungeonScene : Helper.Singleton<DungeonScene>
    {
        ...
        public void Random_Monster()
        {
            ...
            for (int i = 0; i < monsterType; i++)
            {
                ...
                if (Rand < 5)
                {
                    monsters[i] = new Monster("맼닠젘", 12, 50, 50, 10, 25);
                }
                else if (Rand < 20)
                {
                    monsters[i] = new Monster("오크", 7, 26, 26, 8, 4);
                }
                else if (Rand < 50)
                {
                    monsters[i] = new Monster("고블린", 5, 22, 22, 3, 5);
                }
                else
                {
                    monsters[i] = new Monster("슬라임", 3, 15, 15, 1, 1);
                }
            }
        }
            ...
            for (int i = 0; i < monsters.Length; i++)
            {
                if(monsters[i] != null)
                {
                    if (monsters[i].IsDead == false)
                    {
                        ...
                        // 플레이어 체력 깎아주기
                        player.HP = (int)(player.HP - AttRangeDamage);

                        Utill.ColorWriteLine($"{monsters[i].Name} 공격", ConsoleColor.Red);
                        Utill.ColorWriteLine($"{player.Name}는(은) {AttRangeDamage}의 데미지를 받았다.\n");
